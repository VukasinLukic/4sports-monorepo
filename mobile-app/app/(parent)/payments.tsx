...
export default function ParentPayments() {
  ...
  const fetchPayments = useCallback(async () => {
    try {
      ...
      // Then fetch payments for each child
      const allPayments: Payment[] = [];
      for (const child of children) {
        try {
          const paymentsResponse = await api.get(`/payments/member/${child._id}`);
          const childPayments = (paymentsResponse.data.data || []).map((p: any) => ({
            ...p,
            memberId: { _id: child._id, fullName: child.fullName },
          }));
          allPayments.push(...childPayments);
        } catch {
          // Skip if no payments
        }
      }
    ...
    <Card style={styles.paymentCard}>
      <Card.Content>
        <View style={styles.paymentHeader}>
          <View>
            <Text style={styles.memberName}>{item.memberId.fullName}</Text>
