...
interface ClubInfo {
  ...
  email?: string;
  description?: string;
  ownerId: string | { _id: string; fullName: string; email: string; phoneNumber?: string };
}
...
export default function ClubInfoScreen() {
  ...
  const fetchClubData = useCallback(async () => {
    ...
    try {
      ...
      // Fetch all users in the club to get coaches and owner
      const usersResponse = await api.get('/chat/users');
      const users = usersResponse.data.data || [];

      // Separate owner and coaches
      ...
      <Card style={styles.clubCard}>
        <Card.Content style={styles.clubContent}>
          ...
          {clubInfo?.description && (
            <Text style={styles.clubDescription}>{clubInfo.description}</Text>
          )}
        </Card.Content>
      </Card>
      ...
      {coaches.length > 0 && (
        <View style={styles.section}>
          <Text style={styles.sectionHeader}>
            {t('roles.coach')}s ({coaches.length})
          </Text>
